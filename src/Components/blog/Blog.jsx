...
const Blog = () => {
  ...
  return (
    <Layout>
      ...
      <div className="flex flex-col mb-40 mt-30 md:mt-0 gap-20 justify-center items-center">
        <div className="p-6 w-full max-w-6xl">
          {/* Blog Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2  gap-4 justify-items-center">
            {blogs.slice(0, 6).map((blog, index) => (
              <a href={`/blog/${slugify(blog.blog_title)}`} state={{ blog }} key={index} className="w-full">
              <Card
                key={index}
                className="overflow-hidden shadow-md w-full max-w-lg"
              >
